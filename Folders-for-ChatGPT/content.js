const SVG_unpin =
  ...
  <div class="group relative rounded-lg active:opacity-90">
    <span class="flex items-center gap-2 p-2 text-sm text-token-text-tertiary">${chrome.i18n.getMessage(
      'noPinnedChats'
    )}</span>
  </div>
</li>`;
...
class UIService {
  ...
  async bindPinnedList() {
    const e = document.querySelector('nav').querySelector('h3').parentElement
  ...
  async bindPinUnpinCurrentChatButton() {
    if (window.location.pathname.match(/^\/c\/[0-9a-f-]{36}$/)) {
      ...
      r.title = s
        ? chrome.i18n.getMessage('unpin')
        : chrome.i18n.getMessage('pin');
      r.innerHTML = s ? `${SVG_unpin} Pinned` : `${SVG_pin} Pin`;
      r.onclick = async () => {
        this.dbService.toggleChatPin(n, document.title);
        r.title =
          r.title === chrome.i18n.getMessage('pin')
            ? chrome.i18n.getMessage('unpin')
            : chrome.i18n.getMessage('pin');
        r.innerHTML =
          r.title === chrome.i18n.getMessage('pin')
  ...
  async insertPinUnpinButton(e) {
    ...
    setTimeout(async () => {
      ...
      i.textContent = n
        ? `${chrome.i18n.getMessage('unpin')} Pinned`
        : `${chrome.i18n.getMessage('pin')} Pin`; // Show "Pinned" or "Pin"
      r.innerHTML = n ? SVG_unpin : SVG_pin;
      i.insertBefore(r, i.firstChild);
      i.onclick = async () => {
        await this.dbService.toggleChatPin(e, t);
        i.textContent = i.textContent.includes('Pin')
          ? `${chrome.i18n.getMessage('unpin')} Pinned`
  ...
  updatePinnedChats() {
    this.dbService.getPinnedChats().then((e) => {
      ...
      i.querySelector('h3').textContent = `${chrome.i18n.getMessage(
        'pinned'
      )} Pinned`; // Update the header text
      ...
      e.forEach(({ id: e, name: t }) => {
        ...
        const o = document.createElement('button'),
          c =
            'flex items-center justify-center text-token-text-primary transition hover:text-token-text-secondary radix-state-open:text-token-text-secondary juice:text-token-text-secondary juice:hover:text-token-text-primary hidden group-hover:flex'.split(
              ' '
            );
        o.classList.add(...c);
  ...
  bindCloseModal() {
    document.addEventListener(
      ...
      (e) => {
        if (
          e.target.closest('#' + dialogCloseID) ||
          e.target.id === overlayBackdropID
        ) {
...
class DBService {
  ...
  async getPinnedChats() {
    return (
      (await chrome.storage.sync.get(DBService.PINNED_CHATS_KEY))[
        DBService.PINNED_CHATS_KEY
      ] || []
    );
  }
  setPinnedChats(e) {
...
dbService.setOnChangedCallback(() => {
  try {
    // Update pinned chats immediately
    uiService.updatePinnedChats();

    // Schedule updates after 1 second and 4 seconds
    setTimeout(() => uiService.updatePinnedChats(), 1000); // 1 second
    setTimeout(() => uiService.updatePinnedChats(), 4000); // 4 seconds
  } catch (error) {
...
new MutationObserver((e, t) => {
  e.forEach((e) => {
    e.addedNodes.forEach((e) => {
      e?.querySelector &&
        e.querySelector('img[alt="User"]') &&
        uiService.bindPinUnpinCurrentChatButton();
    });
  });
}).observe(document.body, {
  childList: !0,
